...
public class MB_TextureCombinerRenderTexture{
    ...
    */
	...
	private void CopyScaledAndTiledToAtlas(MB_TexSet texSet, MeshBakerMaterialTexture source, Vector2 obUVoffset, Vector2 obUVscale, Rect rec, ShaderTextureProperty texturePropertyName, TextureBlender resultMatTexBlender){			
        ...
        {
            myCamera.backgroundColor = MB3_TextureCombinerNonTextureProperties.GetColorIfNoTexture(texturePropertyName);
        }
...
public class MB3_AtlasPackerRenderTexture : MonoBehaviour {
	...
	public Texture2D OnRenderAtlas(MB3_TextureCombiner combiner){
        ...
        if (resultMaterialTextureBlender != null) blender = resultMaterialTextureBlender.GetTextureBlender();
