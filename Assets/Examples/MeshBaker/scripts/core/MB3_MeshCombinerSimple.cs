        ...
        void _copyAndAdjustUVsFromMesh(MB_DynamicGameObject dgo, Mesh mesh, int vertsIdx, MeshChannelsCache meshChannelsCache)
        {
            ...
            if (needToModfyUVs)
            {
                ...
                for (int k = 0; k < dgo.targetSubmeshIdxs.Length; k++)
                {
                    ...
                    Rect rr = MB3_TextureCombinerMerging.BuildTransformMeshUV2AtlasRectB(textureBakeResults.resultMaterials[dgo.targetSubmeshIdxs[k]].considerMeshUVs,
                                dgo.uvRects[k],
                                dgo.obUVRects == null ? new Rect(0,0,1,1) : dgo.obUVRects[k],
                                dgo.sourceMaterialTiling[k],
                                dgo.encapsulatingRect[k]);
